...
// 调型折线：5 个音高采样(1..5) → SVG 坐标，画参考格 + 实线 + 起讫点。
...
export function ChineseClassroom() {
  ...
  const t = {
    ...
    tryLabel: { en: "Hear the four tones + neutral", zh: "先听四声加轻声" },
  ...
  return (
    <div className="bg-paper">
      <div
        aria-hidden
        className="shot-hide fixed left-0 top-16 z-40 h-[3px] bg-gradient-to-r from-orange to-teal transition-[width] duration-150 ease-out"
        style={{ width: `${progress * 100}%` }}
      />
      {/* HERO：左对齐，旁边直接放可听声调试听——第二屏前就能听到中文 */}
      <section ref={heroRef} className="relative isolate overflow-hidden pt-28 pb-16 sm:pt-32 sm:pb-20">
        <div aria-hidden className="pointer-events-none absolute inset-0">
          ...
          <div
            ...
            style={{
              backgroundImage: "radial-gradient(rgb(45 106 79 / 0.06) 1px, transparent 1px)",
              backgroundSize: "26px 26px",
              maskImage: "radial-gradient(ellipse 80% 60% at 70% 30%, black, transparent)",
              WebkitMaskImage: "radial-gradient(ellipse 80% 60% at 70% 30%, black, transparent)",
            }}
          />
        </div>
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid items-center gap-12 lg:grid-cols-[1.1fr_0.9fr]">
            <div>
              ...
              <Reveal delay={80}>
                <h1 className="display-zh text-ink !text-[clamp(2.6rem,7vw,5rem)] !leading-[0.98]">{t.title[lang]}</h1>
              </Reveal>
              <Reveal delay={150}>
                <p className="mt-6 max-w-xl text-lg leading-relaxed text-ink-light">{t.intro[lang]}</p>
              </Reveal>
              <Reveal delay={220}>
                <a href="#tones" className="btn-primary mt-8 inline-flex text-base">
                  {t.startCta[lang]} <IconArrowRight size={18} />
                </a>
              </Reveal>
            </div>

            {/* 可听声调试听卡：mā má mǎ mà ma，带迷你调型，点一下就读 */}
            <Reveal delay={160}>
              <div className="rounded-2xl border border-teal/12 bg-white/80 p-6 shadow-[0_30px_70px_-40px_rgb(31_74_56/0.5)] backdrop-blur-sm sm:p-7">
                <p className="font-inter text-xs font-semibold uppercase tracking-[0.14em] text-orange">{t.tryLabel[lang]}</p>
      ...
      <section className="px-4 pb-24 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[15rem_1fr]">
                ...
                {MODULES.map((m, i) => {
                  ...
                  return (
                    <li key={m.id}>
                      <a
                        href={`#${m.id}`}
                        className={`-ml-px flex items-center gap-3 border-l-2 py-2 pl-4 text-sm transition-colors ${
                          on ? "border-orange font-semibold text-ink" : "border-transparent text-ink-light hover:border-teal/40 hover:text-teal"
                        }`}
                      >
          ...
          <div className="min-w-0 space-y-20 sm:space-y-24">
            {/* 01 声调 */}
            <Reveal>
              <section id="tones" className="scroll-mt-24">
                <SectionHead
                  index="01"
                  en="The four tones + neutral"
                  zh="四个声调 + 轻声"
                  lead={{
                    en: "Mandarin is tonal: the pitch shape changes the word. Learn the shape, the feel, and a hand gesture for each — then tap examples to hear them.",
                    zh: "普通话是有声调的：音高怎么走，字就不同。先记住每个调的形状、感觉和一个手势，再点例字听。",
                  }}
                />
            ...
            {/* 02 声母 */}
            <Reveal>
              <section id="initials" className="scroll-mt-24">
                <SectionHead
                  index="02"
                  en="Initials — the consonant onsets"
                  zh="声母——字音开头的辅音"
                  lead={{
                    en: "21 onsets. The small character is how teachers say the letter aloud; tap it to hear the sound, then read the letter.",
                    zh: "21 个声母。旁边的小字是老师念这个字母的音，点它听声，再认字母。",
                  }}
                />
                <div className="grid grid-cols-3 gap-2.5 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6">
                  {initials.map((w) => (
                    <div key={w.pinyin} className="group/in flex items-center justify-between gap-2 rounded-xl border border-teal/12 bg-white px-3 py-2.5 transition-all duration-200 hover:-translate-y-0.5 hover:border-teal/40">
            ...
            {/* 03 韵母 */}
            <Reveal>
              <section id="finals" className="scroll-mt-24">
                <SectionHead
                  index="03"
                  en="Finals — the vowel cores"
                  zh="韵母——字音里的元音核心"
                  lead={{
                    en: "24 finals carry the open sound of a syllable. Tap the example character to hear each one clearly.",
                    zh: "24 个韵母，承载一个音节里敞开的声音。点代表字，听清每一个。",
                  }}
                />
            ...
            {/* 04 拼读 */}
            <Reveal>
              <section id="blends" className="scroll-mt-24">
                <SectionHead
                  index="04"
                  en="Blend them together"
                  zh="把它们拼起来"
                  lead={{
                    en: "An initial plus a final makes a syllable. Read the two parts, then tap the result to hear the real character.",
                    zh: "声母加韵母，拼成一个音节。先读两部分，再点结果听真实的字。",
                  }}
                />
            ...
            {/* 05 笔画 */}
            <Reveal>
              <section id="strokes" className="scroll-mt-24">
                <SectionHead
                  index="05"
                  en="Strokes — how a character is built"
                  zh="笔画——一个字怎样写出来"
                  lead={{
                    en: "Every character is a few strokes in order. Hover a card to watch the stroke write itself; tap the example to hear it.",
                    zh: "每个字都是几笔、按顺序写成。把鼠标移到卡片上看它一笔写出来，点例字听音。",
                  }}
                />
                  ...
                  {strokes.map((s) => (
                    <div key={s.en} className="group/stroke rounded-2xl border border-teal/12 bg-white p-4 transition-all duration-200 hover:-translate-y-1 hover:border-teal/40 hover:shadow-[0_16px_34px_-22px_rgb(31_74_56/0.5)]">
                      ...
                      <div className="mt-2 flex items-center justify-between gap-2">
                        <div>
                          <p className="font-nunito text-base font-bold text-ink">{s.name[lang]}</p>
                      ...
                      <p className="mt-2 font-serif-sc text-lg text-ink">
                        {s.example.char} <span className="text-xs text-ink-light">{s.example.pinyin}</span>
                      </p>
                    </div>
                  ))}
                </div>
            ...
            {/* 06 偏旁 */}
            <Reveal>
              <section id="radicals" className="scroll-mt-24">
                <SectionHead
                  index="06"
                  en="Radicals — the meaning clues"
                  zh="偏旁——藏在字里的意思线索"
                  lead={{
                    en: "A radical hints at what a character is about. See the clue, then tap the family of characters that share it.",
                    zh: "偏旁提示一个字和什么有关。先看线索，再点同一偏旁的一族字。",
                  }}
                />
                <div className="grid gap-4 sm:grid-cols-2">
                  {radicals.map((r) => (
            ...
            {/* 07 第一批字 */}
            <Reveal>
              <section id="chars" className="scroll-mt-24">
                <SectionHead
                  index="07"
                  en="Your first characters"
                  zh="你的第一批字"
                  lead={{
                    en: "Sixteen high-frequency characters, ordered from one stroke up. Tap to hear, note the stroke count, recognise the shape.",
                    zh: "16 个高频字，从一画排起。点一下听，记笔画数，认字形。",
                  }}
                />
            ...
            <Reveal>
              <section id="words" className="scroll-mt-24">
                <SectionHead
                  index="08"
                  en="Your first words — say them out loud"
                  zh="你的第一批词——大声说出来"
                  lead={{
                    en: "Twenty words you can use today. Hear each one, then say it back. This is the point of the whole page.",
                    zh: "20 个今天就能用的词。听一个，跟一个。这才是整页的目的。",
                  }}
                />
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {firstWords.map((w) => (
                    <WordChip key={w.char} word={w} showEn />
                  ))}
                </div>
              </section>
            </Reveal>
      ...
      <section className="section-teal px-4 py-20 sm:px-6 sm:py-24 lg:px-8">
        <Reveal>
          <div className="mx-auto flex max-w-5xl flex-col items-start gap-8 md:flex-row md:items-center md:justify-between">
            ...
            <Link href="/#learn-paths" className="btn-accent shrink-0 text-base">
              {t.bridgeCta[lang]} <IconArrowRight size={18} />
            </Link>
          </div>
        </Reveal>
      </section>
